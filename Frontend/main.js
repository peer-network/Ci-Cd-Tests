  ...
  return fetch(apiUrl, {
    ...
    .then((result) => {
      if (result.data.contactus.status === 'success') {
        ...
        throw new Error(
          'Error submitting the form: ' + result.data.contactus.ResponseCode
        );
      }
    })
    .catch((error) => {
    ...
    if (validateInputs()) {
      try {
        if (
          ...
          messageInput.value = '';
          alert(
            'We have received your message, thank you very much for your feedback, and wishing you much success with Peernetwork.'
          );
        }
      } catch (error) {
